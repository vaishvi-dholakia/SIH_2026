...
let dynamicMainlandPolygon = MAINLAND_INDIA_POLYGON;
let dynamicIslandBounds = ISLAND_BOUNDS;

export function setIndiaBoundaryData(data) {
  if (data && data.mainland_polygon && Array.isArray(data.mainland_polygon) && data.mainland_polygon.length > 0) {
    dynamicMainlandPolygon = data.mainland_polygon;
  }
  if (data && data.island_bounds && Array.isArray(data.island_bounds) && data.island_bounds.length > 0) {
    dynamicIslandBounds = data.island_bounds;
  }
}
...
export function isPointInIndia(latitude, longitude) {
  ...
  const polygon = dynamicMainlandPolygon;
  const islands = dynamicIslandBounds;

  // Check Island Bounds
  for (const [minLon, minLat, maxLon, maxLat] of islands) {
